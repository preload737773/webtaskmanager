...
export class DragAndDropPanel {
    private readonly divField: HTMLElement;
    private readonly passiveSupported: boolean;
    private _maxItemWidth: number;
    private readonly _copyNodeLabels: boolean;
    private _beginDragCallback: any;
     ...
     */
    constructor(div: HTMLElement, passiveSupported: boolean) {
        this.divField = div;
        this._maxItemWidth = 200;
        this.passiveSupported = passiveSupported;
        this._copyNodeLabels = true;
    }
    ...
    get beginDragCallback() {
        return this._beginDragCallback;
    }

    set beginDragCallback(callback) {
        this._beginDragCallback = callback;
    }
     ...
     */
    createNodeVisual(original: { element: any; tooltip: string; }, graphComponent: GraphComponent) : HTMLDivElement {
        ...
        originalNode.ports.forEach((port: IPort) => {
     ...
     * @return {HTMLDivElement}
     */
    createEdgeVisual(original: { element: any; tooltip: string; }, graphComponent: GraphComponent) : HTMLDivElement {
        const graph = graphComponent.graph;
        graph.clear();
    ...
    updateViewport(graphComponent: GraphComponent) {
        const graph = graphComponent.graph;
     ...
     */
    addPointerDownListener(item: INode, element: HTMLElement, callback: any) {
        if (!callback) {
            return
        }
        ...
        const doDragOperation = () => {
            if (typeof IStripe !== 'undefined' && IStripe.isInstance(item.tag)) {
                ...
                simpleNode.labels = this._copyNodeLabels ? item.labels : IListEnumerable.EMPTY
        ...
        const touchStartListener = (evt: Event | PointerEvent) => {
            doDragOperation()
            evt.preventDefault()
        }
